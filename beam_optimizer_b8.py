...
# -*- coding: utf-8 -*-
"""
beam_optimizer_b8.py

README / 実行方法
-----------------
- 前提: Python 3.x, openpyxl
- 実行例:
    python beam_optimizer_b8.py input.xlsx output.xlsx
...
def solve_layout(
    ...
    - MAIN beams: section can vary per beam (choose minimum weight section that passes)
    - TRANS beams: added only when a point load is off the MAIN beam line; TRANS section also selected per member
    """
    ...
    for td in transfer_defs:
        ...
        pts = [(td.P, td.a_tb)]  # only the original point load, self-weight is in w_udl
        ...
        # reactions include self-weight (w_udl = chosen.w_udl)
...
def optimize(
    ...
    """
    Mode-A only:
      - For each (direction, pitch), select sections per member to minimize member weight.
      - Choose the (direction, pitch) that minimizes total_weight.
    """
    ...
    # progress
    total_cases = len(directions) * len(pitches)
    done = 0
    ok_cnt = 0
    ...
    for d in directions:
        ...
        for pitch in pitches:
            ...
            sol = solve_layout(cfg, mat, setts, sections, d, pitch)
            ...
            if sol.ok:
                ok_cnt += 1
                if best is None or sol.total_weight < best.total_weight - 1e-12 or (
                    abs(sol.total_weight - best.total_weight) <= 1e-12 and sol.util_max < best.util_max
                ):
                    best = sol
                    print(f"[Best updated] dir={sol.direction} pitch={sol.pitch} W={sol.total_weight:.3f} kN maxR={sol.max_rank_used} util={sol.util_max:.3f}")
...
def draw_plan(ws, top: int, left: int, cfg: Config, sol: Solution, title: str) -> int:
    ...
    for td in sol.transfer_defs:
        ...
        label = f"{td.tb_id} u={u:.2f}" if u is not None else f"{td.tb_id}"
...
def main(argv: List[str]) -> int:
    ...
    # run
    cand_rows, best = optimize(cfg, mat, setts, sections)

    # write excel
    write_result_xlsx(in_path, out_path, cfg, mat, setts, cand_rows, best)
