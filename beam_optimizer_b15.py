...
def solve_layout_grid(
    ...
    """
    Two-direction (grid) candidate.
    Slab distributed load q and point loads P are split 50/50 to X/Y systems.
    """
    cfg_half = _scale_cfg_loads(cfg, q_factor=0.5, p_factor=0.5)
    sol_x = solve_layout(cfg_half, mat, setts, sections, direction="X", pitch=pitch_x, cand_id=f"{cand_id}_GX")
    if not sol_x.ok:
        return replace(sol_x, direction="XY", pitch=pitch_x, pitch_y=pitch_y, system="GRID", ng_reason=f"GRID-X failed: {sol_x.ng_reason}")
    sol_y = solve_layout(cfg_half, mat, setts, sections, direction="Y", pitch=pitch_y, cand_id=f"{cand_id}_GY")
    if not sol_y.ok:
        return replace(sol_y, direction="XY", pitch=pitch_x, pitch_y=pitch_y, system="GRID", ng_reason=f"GRID-Y failed: {sol_y.ng_reason}")
    return _merge_direction_solutions(sol_x, sol_y, pitch_x=pitch_x, pitch_y=pitch_y, system="GRID")
...
def solve_layout_grid_positions(
    ...
    sol_x = solve_layout_positions(cfg_half, mat, setts, sections, direction="X", positions=positions_x, cand_id=f"{cand_id}_GX")
    if not sol_x.ok:
        return replace(
            sol_x,
            direction="XY",
            pitch=_calc_layout_pitch_from_positions(cfg, "X", positions_x),
            pitch_y=_calc_layout_pitch_from_positions(cfg, "Y", positions_y),
            system=system,
            ng_reason=f"{system}-X failed: {sol_x.ng_reason}",
        )

    sol_y = solve_layout_positions(cfg_half, mat, setts, sections, direction="Y", positions=positions_y, cand_id=f"{cand_id}_GY")
    if not sol_y.ok:
        return replace(
            sol_y,
            direction="XY",
            ...
            pitch_y=_calc_layout_pitch_from_positions(cfg, "Y", positions_y),
            system=system,
            ng_reason=f"{system}-Y failed: {sol_y.ng_reason}",
        )

    pitch_x = _calc_layout_pitch_from_positions(cfg, "X", positions_x)
    pitch_y = _calc_layout_pitch_from_positions(cfg, "Y", positions_y)
    return _merge_direction_solutions(sol_x, sol_y, pitch_x=pitch_x, pitch_y=pitch_y, system=system)
...
def solve_layout_grid_free(
    ...
    return _merge_direction_solutions(sol_x, sol_y, pitch_x=pitch_x, pitch_y=pitch_y, system="FREE")
...
def prepare_stage(cfg: Config, sections: List[Section]) -> Tuple[List[CandidateSpec], List[Section]]:
    ...
    if cfg.enable_xy_grid and cfg.enable_x and cfg.enable_y and pitches_x and pitches_y:
        lim = float(cfg.short_pitch_limit)
        snap = max(cfg.snap_tol, 1e-6)
...
def write_result_xlsx(
    ...
    ws.cell(
        row=r,
        column=1,
        value="Notes: Units are kN, m. E/fb/fv are converted from N/mm2 to kN/m2. Mode-B is removed. GRID/GRID_LOAD/FREE(XY) candidates split slab q and point P equally to X/Y systems.",
...
def main(argv: List[str]) -> int:
    if len(argv) < 2:
        print("Usage: python beam_optimizer_b15.py input.xlsx output.xlsx [max_workers]")
        return 2
