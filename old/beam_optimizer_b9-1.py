...
def optimize(
    ...
    for d in directions:
        ...
        for pitch in pitches:
            ...
            sol = solve_layout(cfg, mat, setts, sections, d, pitch, cand_id=cand_id)
...
def main(argv: List[str]) -> int:
    ...
    # clear verbose collectors
    _clear_debug()
    trace("RUN_START", "Start optimization run", in_path=in_path, out_path=out_path, Lx=cfg.Lx, Ly=cfg.Ly, q=cfg.q, load_share_model=cfg.load_share_model)
    ...
    trace("RUN_OPT_DONE", "Optimization finished", n_candidates=len(cand_rows), best_ok=(best.ok if best else False), best_direction=(best.direction if best else None), best_pitch=(best.pitch if best else None))
