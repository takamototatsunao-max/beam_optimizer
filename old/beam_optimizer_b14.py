...
def _optimize_direction_free(
    ...
    pitches = _allowed_pitches_for_direction(cfg, direction)
    if not pitches:
        return None, []
    ...
    if not _positions_meet_short_side_limit(cfg, direction, pos):
        return None, pos
    ...
    while improved and it < max_iter and len(pos) > 1:
        ...
        for i in range(len(pos)):
            ...
            if not trial_pos:
                continue
            if not _positions_meet_short_side_limit(cfg, direction, trial_pos):
                continue
...
def solve_layout_grid_free(
    ...
    sol_x, pos_x = _optimize_direction_free(cfg_half, mat, setts, sections, "X", seed_pitch=seed_pitch_x, cand_id=f"{cand_id}_GX")
    ...
    sol_y, pos_y = _optimize_direction_free(cfg_half, mat, setts, sections, "Y", seed_pitch=seed_pitch_y, cand_id=f"{cand_id}_GY")
...
def prepare_stage(cfg: Config, sections: List[Section]) -> Tuple[List[CandidateSpec], List[Section]]:
    ...
    if cfg.enable_xy_grid and cfg.enable_x and cfg.enable_y and pitches_x and pitches_y:
        ...
        free_x = _select_seed_pitches(pitches_x, max_n=2)
        free_y = _select_seed_pitches(pitches_y, max_n=2)
        for sx in free_x:
            for sy in free_y:
                cand_no += 1
                specs.append(CandidateSpec(
...
def write_result_xlsx(
    ...
    ws.cell(
        row=r,
        column=1,
        value="Notes: Units are kN, m. E/fb/fv are converted from N/mm2 to kN/m2. Mode-B is removed. GRID/FREE(XY) candidates split slab q and point P equally to X/Y systems.",
