...
def read_input_xlsx(path: str) -> Tuple[Config, Material, SolverSettings, List[Section]]:
    ...
    cfg = Config(
        ...
        edge_beams=bool(edge_beams), load_rule=int(load_rule),
...
def draw_plan(ws, top: int, left: int, cfg: Config, best: CandidateResult) -> int:
    ...
    def draw_trib_boundaries():
        if len(beams) <= 1:
            return
        poss = [b.pos for b in beams]
        mids = [(poss[i] + poss[i+1]) * 0.5 for i in range(len(poss) - 1)]
        if best.direction == "X":
            for ym in mids:
    ...
    ws.cell(row=leg_r + 1, column=left, value="Beam: light fill + medium line, Tributary boundary: dotted line, Point load: red ●ID").font = font_small
