...
# -*- coding: utf-8 -*-
"""
beam_optimizer_b17.py

README / 実行方法
-----------------
- 前提: Python 3.x, openpyxl
- 実行例:
    python beam_optimizer_b17.py input.xlsx output.xlsx
...
def write_all_cases_pdf(
    ...
    for i, rc in enumerate(render_cases, start=1):
        ...
        col_widths = [8 * mm, 8 * mm, 7 * mm, 10 * mm, 8 * mm, 16 * mm, 29 * mm, 29 * mm]
        header_h = 7.0
        row_h = 7.0
        tbl_top = y
...
def draw_plan(ws, top: int, left: int, cfg: Config, sol: Solution, title: str) -> int:
    ...
    def _safe_put_label(rr: int, cc: int, text: str, font: Optional[Font] = None) -> None:
        """
        Put label text without creating merged cells.
        If target is occupied, try nearby cells on the same row.
        """
        rr0 = max(1, int(rr))
        cc0 = max(1, int(cc))
    ...
    for mc in sol.member_checks:
        ...
        if mc.direction == "Y":
            c = x_to_c(pos)
            _safe_put_label(grid_top - 2, c, label, font_small)
        else:
            r0 = y_to_r(pos)
            # label to the left of the beam row
            _safe_put_label(r0, grid_left - 2, label, font_small)
    ...
    for td in sol.transfer_defs:
        ...
        if td.span_dir == "X":
            ...
            cc0 = max(grid_left + 1, min(grid_left + nx, x_to_c(midx)))
            _safe_put_label(rr, cc0, label, Font(size=8, bold=True))
        else:
            cc = max(grid_left + 1, min(grid_left + nx, x_to_c(midx) + 1))
            rr0 = y_to_r(midy)
            rr = max(grid_top + 1, min(grid_top + ny, rr0))
            _safe_put_label(rr, cc, label, Font(size=8, bold=True))
...
def main(argv: List[str]) -> int:
    if len(argv) < 2:
        print("Usage: python beam_optimizer_b17.py input.xlsx output.xlsx [max_workers]")
        return 2
