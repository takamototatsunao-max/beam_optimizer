...
class Beam:
    beam_id: str
    direction: str  # 'X' or 'Y' (span direction of MAIN beams)
    pos: float      # position along pitch direction (m) for MAIN beams; for TRANS beams this may be NaN
    span: float     # m
    trib_width: float  # m (MAIN beams only; TRANS beams set to 0)
    point_along: List[Tuple[float, float]]  # (P[kN], a[m] from left)

@dataclass(frozen=True)
class TransferBeam:
    """A short secondary beam added only to carry an off-beam point load.

    It spans along the pitch axis (perpendicular to MAIN beams) between two supports:
      - left/right MAIN beam, or perimeter primary beam at 0/width

    Loads on this transfer beam:
      - self-weight only (w_g [kN/m]) as UDL
      - one or more point loads (from floor) at their pitch-coordinate location

    Reactions (including self-weight) are transferred to supports and applied as point loads
    on MAIN beams at the span-coordinate of each point load.
    """
    tb_id: str
    span_dir: str            # 'X' or 'Y' (span direction of TRANS beam)
    fixed_coord: float       # coordinate on the perpendicular axis (m): for X-span => y, for Y-span => x
    left_support_id: str     # MAIN beam_id or 'PERIM'
    right_support_id: str
    left_pos: float          # pitch-axis coordinate (m)
    right_pos: float
    point_loads: List[Tuple[str, float, float]]  # (LoadID, P[kN], a[m] from left support)
...
class BeamCheck:
    beam_id: str
    beam_type: str           # 'MAIN' or 'TRANS'
    direction: str           # span direction 'X' or 'Y'
    pos: Optional[float]     # MAIN: pitch position; TRANS: None
    span: float
    w_udl: float             # kN/m
    n_point: int
    Mmax: float              # kN*m
...
class CandidateResult:
    direction: str
    pitch: float
    n_beams: int                 # number of MAIN beams
    n_transfer: int              # number of TRANS beams (added for off-beam point loads)
    transfer_total_length: float # sum of TRANS beam spans [m]

    section_rank: int
    section_name: str
    w_g: float                   # kN/m (self weight of selected section)

    w_total_max: float           # max UDL [kN/m] on MAIN beams (q*trib + w_g)

    Mmax: float                  # kN*m (max among ALL beams: MAIN + TRANS)
    Vmax: float                  # kN (max among ALL beams: MAIN + TRANS)
    dmax: float                  # m   (max among ALL beams: MAIN + TRANS)
    ...
    total_weight: float          # kN (MAIN + TRANS self weights)
    ok: bool
    ng_reason: str

    allocation_rows: List[Tuple[str, str, float]]  # (LoadID, SupportID, AllocP[kN])  SupportID may be MAIN beam_id or 'PERIM'
...
def build_main_beams(cfg: Config, direction: str, pitch: float) -> List[Beam]:
    """Create MAIN secondary beams (one-way framing) WITHOUT allocating point loads."""
    if direction == "X":
        span = cfg.Lx
        width = cfg.Ly
    else:
...
def build_system(cfg: Config, direction: str, pitch: float, section: Section) -> Tuple[List[Beam], List[TransferBeam], List[Tuple[str, str, float]]]:
    """Build MAIN beams and (if needed) TRANS beams for off-beam point loads.

    Rule implemented (per user requirement):
      - If a point load is located directly on a MAIN beam centerline -> applied to that beam.
      - If not -> create a TRANS beam at that fixed coordinate (y for Y-dir, x for X-dir),
        spanning between the adjacent MAIN beams (or perimeter primary beam), and transfer
        reactions (including TRANS self-weight) to the supports.

    Notes:
      - Perimeter at 0/width is assumed supported by primary beams, so it can be a support
        for TRANS beams but is NOT modeled as a MAIN secondary beam.
      - TRANS beams carry only their self-weight (w_g) + the floor point load(s). No area load.
    """
    main_beams = build_main_beams(cfg, direction, pitch)
    transfer_beams: List[TransferBeam] = []
    allocation_rows: List[Tuple[str, str, float]] = []

    if not main_beams:
        return [], [], []

    # mapping for coordinate axes
    if direction == "X":
        span_L = cfg.Lx
        width = cfg.Ly
        # MAIN beams at y=pos, span coordinate is x
        def pitch_coord(pl: PointLoad) -> float: return pl.y
        def span_coord(pl: PointLoad) -> float: return pl.x
        pitch_axis = "Y"
        trans_span_dir = "Y"  # TRANS spans along pitch axis (Y), fixed x
        def fixed_coord(pl: PointLoad) -> float: return pl.x
    else:
        span_L = cfg.Ly
        width = cfg.Lx
        # MAIN beams at x=pos, span coordinate is y
        def pitch_coord(pl: PointLoad) -> float: return pl.x
        def span_coord(pl: PointLoad) -> float: return pl.y
        pitch_axis = "X"
        trans_span_dir = "X"  # TRANS spans along pitch axis (X), fixed y
        def fixed_coord(pl: PointLoad) -> float: return pl.y

    # MAIN beam positions in pitch axis
    main_pos = [b.pos for b in main_beams]
    snap_tol = 1e-6  # [m] treat as 'on beam' only if essentially coincident

    # helper to add point load to MAIN beam
    beam_by_id = {b.beam_id: b for b in main_beams}

    def find_on_beam(c: float) -> Optional[Beam]:
        for b in main_beams:
            if abs(b.pos - c) <= snap_tol:
                return b
        return None

    def find_adjacent_supports(c: float) -> Tuple[str, float, str, float]:
        # returns (left_id, left_pos, right_id, right_pos), where id may be 'PERIM'
        left_id, left_p = "PERIM", 0.0
        right_id, right_p = "PERIM", width

        # left support
        for b in main_beams:
            if b.pos < c - snap_tol:
                left_id, left_p = b.beam_id, b.pos
            else:
                break
        # right support
        for b in reversed(main_beams):
            if b.pos > c + snap_tol:
                right_id, right_p = b.beam_id, b.pos
            else:
                break
        # If c between two beams, the above finds farthest-left and farthest-right.
        # For sorted positions, we want nearest on each side:
        # fix right by scanning from left
        right_id, right_p = "PERIM", width
        for b in main_beams:
            if b.pos > c + snap_tol:
                right_id, right_p = b.beam_id, b.pos
                break
        # and left by scanning from right
        left_id, left_p = "PERIM", 0.0
        for b in reversed(main_beams):
            if b.pos < c - snap_tol:
                left_id, left_p = b.beam_id, b.pos
        ...
        return left_id, left_p, right_id, right_p
    ...
    for pl in cfg.loads:
        ...
        c = pitch_coord(pl)
        a_span = span_coord(pl)

        # loads on perimeter primary beam (c==0 or c==width) are not assigned to MAIN beams
        if abs(c - 0.0) <= snap_tol or abs(c - width) <= snap_tol:
            allocation_rows.append((pl.load_id, "PERIM", pl.P))
            continue

        on = find_on_beam(c)
        if on is not None:
            on.point_along.append((pl.P, a_span))
            allocation_rows.append((pl.load_id, on.beam_id, pl.P))
            continue

        # Off-beam -> create TRANS beam at fixed_coord, spanning between adjacent supports
        left_id, left_p, right_id, right_p = find_adjacent_supports(c)
        Ltb = right_p - left_p
        if Ltb <= 1e-9:
            # degenerate; send to nearest support
            nearest = min(main_beams, key=lambda b: abs(b.pos - c))
            nearest.point_along.append((pl.P, a_span))
            allocation_rows.append((pl.load_id, nearest.beam_id, pl.P))
            continue

        a_tb = c - left_p  # from left support along TRANS
        tb_id = f"TB_{pl.load_id}"

        # reactions from TRANS: self-weight + point load at a_tb
        w = section.w_g
        RA = w * Ltb / 2.0 + pl.P * (Ltb - a_tb) / Ltb
        RB = w * Ltb / 2.0 + pl.P * a_tb / Ltb

        # apply reactions to MAIN beams if they are supports; perimeter reaction is just recorded
        if left_id != "PERIM":
            beam_by_id[left_id].point_along.append((RA, a_span))
        allocation_rows.append((pl.load_id, left_id, RA))

        if right_id != "PERIM":
            beam_by_id[right_id].point_along.append((RB, a_span))
        allocation_rows.append((pl.load_id, right_id, RB))

        transfer_beams.append(
            TransferBeam(
                tb_id=tb_id,
                span_dir=trans_span_dir,
                fixed_coord=fixed_coord(pl),
                left_support_id=left_id,
                right_support_id=right_id,
                left_pos=left_p,
                right_pos=right_p,
                point_loads=[(pl.load_id, pl.P, a_tb)],
            )
        )

    return main_beams, transfer_beams, allocation_rows
...
def evaluate_candidate(
    ...
    main_beams, transfer_beams, alloc_rows = build_system(cfg, direction, pitch, section)
    ...
    if not main_beams:
        return CandidateResult(
            ...
            total_weight=0.0,
            ok=False,
            ng_reason="no secondary beams generated (check pitch and perimeter/edge setting)",
            allocation_rows=[],
        )

    # section props (approx if not provided)
    _, Av, Z, I = get_section_props_m(section)

    prev_key = None
    ng_reason = ""

    Mmax_all = Vmax_all = dmax_all = 0.0
    utilM_all = utilV_all = utild_all = 0.0
    w_total_max_main = 0.0

    # NOTE: with fixed section, w_g is constant => one iteration is sufficient.
    for _it in range(1, max(setts.max_iter, 1) + 1):
        Mmax_all = Vmax_all = dmax_all = 0.0
        utilM_all = utilV_all = utild_all = 0.0
        w_total_max_main = 0.0

        # MAIN beams: area load + self weight + transferred reactions
        for b in main_beams:
            w_q = cfg.q * b.trib_width
            w_total = w_q + section.w_g
            w_total_max_main = max(w_total_max_main, w_total)
        ...
        # TRANS beams: self weight + floor point load(s) only
        for tb in transfer_beams:
            Ltb = tb.span
            if Ltb <= 1e-9:
                continue
            pts = [(float(P), float(a)) for _lid, P, a in tb.point_loads]
            Mmax, Vmax, dmax = analyze_simply_supported(
                L=Ltb,
                w_udl=section.w_g,
                point_loads=pts,
            ...
            utilM = (Mmax / Z) / mat.fb_kN_m2
            utilV = (Vmax / Av) / mat.fv_kN_m2
            utilD = dmax / (Ltb / mat.deflection_limit)
    ...
    # weights
    span_main = main_beams[0].span
    n_main = len(main_beams)
    trans_total_length = sum(tb.span for tb in transfer_beams)
    total_weight = section.w_g * span_main * n_main + section.w_g * trans_total_length

    return CandidateResult(
        direction=direction,
        pitch=pitch,
        n_beams=n_main,
        n_transfer=len(transfer_beams),
        transfer_total_length=trans_total_length,
        section_rank=section.rank,
        section_name=section.name,
        w_g=section.w_g,
        w_total_max=w_total_max_main,
        Mmax=Mmax_all,
        Vmax=Vmax_all,
        dmax=dmax_all,
        util_M=utilM_all,
...
def compute_system_checks(
    ...
    pitch: float,
    section: Section
) -> Tuple[List[Beam], List[TransferBeam], List[Tuple[str, str, float]], Dict[str, BeamCheck], str]:
    """Build the framing system and return per-beam checks for LAYOUT output."""
    main_beams, transfer_beams, alloc_rows = build_system(cfg, direction, pitch, section)
    _, Av, Z, I = get_section_props_m(section)

    checks: Dict[str, BeamCheck] = {}
    worst_id = ""
    worst_util = -1.0

    # MAIN beams
    for b in main_beams:
        w_udl = cfg.q * b.trib_width + section.w_g
        Mmax, Vmax, dmax = analyze_simply_supported(
            L=b.span,
            w_udl=w_udl,
            point_loads=b.point_along,
        ...
        bc = BeamCheck(
            beam_id=b.beam_id, beam_type="MAIN", direction=b.direction, pos=b.pos,
            span=b.span, w_udl=w_udl, n_point=len(b.point_along),
            Mmax=Mmax, Vmax=Vmax, dmax=dmax,
            util_M=utilM, util_V=utilV, util_d=utilD, util_max=utilMax
        )
        checks[b.beam_id] = bc
        if utilMax > worst_util:
            worst_util = utilMax
            worst_id = b.beam_id
    ...
    for tb in transfer_beams:
        ...
        bc = BeamCheck(
            beam_id=tb.tb_id, beam_type="TRANS", direction=tb.span_dir, pos=None,
            span=Ltb, w_udl=section.w_g, n_point=len(tb.point_loads),
            Mmax=Mmax, Vmax=Vmax, dmax=dmax,
            util_M=utilM, util_V=utilV, util_d=utilD, util_max=utilMax
        )
        checks[tb.tb_id] = bc
    ...
    return main_beams, transfer_beams, alloc_rows, checks, worst_id
...
def draw_plan(
    ...
    """Draw a cell-based plan view and return the next available row."""
    grid = 0.25  # [m/cell]
    nx = int(round(cfg.Lx / grid)) + 1
    ny = int(round(cfg.Ly / grid)) + 1

    # Styles
    fill_main = PatternFill("solid", fgColor="DDEBF7")   # light blue
    fill_trans = PatternFill("solid", fgColor="FFF2CC")  # yellow
    fill_worst = PatternFill("solid", fgColor="F8CBAD")  # light red
    fill_blank = PatternFill("solid", fgColor="FFFFFF")

    border_thick = Border(left=Side(style="thick"), right=Side(style="thick"),
                          top=Side(style="thick"), bottom=Side(style="thick"))
    border_med_right = Border(right=Side(style="medium"))
    border_med_bottom = Border(bottom=Side(style="medium"))
    ...
    def x_to_c(xm: float) -> int:
        return left + 3 + int(round(xm / grid))

    def y_to_r(ym: float) -> int:
        return top + 4 + int(round(ym / grid))

    # Title
    ws.cell(row=top, column=left, value="LAYOUT / PLAN VIEW (cells only)").font = Font(bold=True, size=14)

    # Plan summary line
    ws.cell(row=top + 2, column=left, value=f"Mode={best.mode_label} best ({best.mode_desc})")
    ws.cell(row=top + 3, column=left, value=f"PLAN: {best.direction}-dir, pitch={best.pitch} m, sec={best.section_name}")

    # row heights / col widths
    ws.row_dimensions[top + 2].height = 42  # for beam labels if any
    for rr in range(top + 4, top + 4 + ny):
        ws.row_dimensions[rr].height = 10
    for cc in range(left + 2, left + 2 + nx + 2):
        ws.column_dimensions[get_column_letter(cc)].width = 2.2
    ...
    for c in range(c0, c1 + 1):
        ws.cell(row=r0, column=c).border = Border(top=Side(style="thick"))
        ws.cell(row=r1, column=c).border = Border(bottom=Side(style="thick"))
    for r in range(r0, r1 + 1):
        ws.cell(row=r, column=c0).border = Border(left=Side(style="thick"))
        ws.cell(row=r, column=c1).border = Border(right=Side(style="thick"))

    # Axis labels (meters)
    ws.cell(row=top + 4 - 1, column=left, value="y[m]→x[m]").alignment = Alignment(horizontal="left")
    for xm in range(int(cfg.Lx) + 1):
        ws.cell(row=top + 4 - 1, column=x_to_c(xm), value=xm).alignment = Alignment(horizontal="center")
    for ym in range(int(cfg.Ly) + 1):
        ws.cell(row=y_to_r(ym), column=left + 2, value=ym).alignment = Alignment(horizontal="center")

    # Tributary dotted boundaries (same as before)
    if best.direction == "Y":
        # boundaries in X
        support = [0.0] + [b.pos for b in main_beams] + [cfg.Lx]
        mids = [(support[i] + support[i + 1]) / 2.0 for i in range(len(support) - 1)]
        for xm in mids:
            cc = x_to_c(xm)
            for rr in range(r0, r1 + 1):
                ws.cell(row=rr, column=cc).border = Border(left=Side(style="dotted"))
    else:
        # boundaries in Y
        support = [0.0] + [b.pos for b in main_beams] + [cfg.Ly]
        mids = [(support[i] + support[i + 1]) / 2.0 for i in range(len(support) - 1)]
        for ym in mids:
            rr = y_to_r(ym)
            for cc in range(c0, c1 + 1):
                ws.cell(row=rr, column=cc).border = Border(top=Side(style="dotted"))

    # Draw MAIN beams (light fill + medium line), highlight worst in red
    for b in main_beams:
        is_worst = (b.beam_id == worst_id)
        f = fill_worst if is_worst else fill_main

        if best.direction == "Y":
            cc = x_to_c(b.pos)
            for rr in range(r0, r1 + 1):
                ws.cell(row=rr, column=cc).fill = f
                ws.cell(row=rr, column=cc).border = border_med_right
            # label with util
            util = checks[b.beam_id].util_max if b.beam_id in checks else float("nan")
            label = f"{b.beam_id}\nx={b.pos:.3g}\nutil={util:.2f}"
            ws.cell(row=top + 2, column=cc, value=label).alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
            ws.cell(row=top + 2, column=cc).font = Font(size=9, bold=True)
        else:
            rr = y_to_r(b.pos)
            for cc in range(c0, c1 + 1):
                ws.cell(row=rr, column=cc).fill = f
                ws.cell(row=rr, column=cc).border = border_med_bottom
            util = checks[b.beam_id].util_max if b.beam_id in checks else float("nan")
            label = f"{b.beam_id} y={b.pos:.3g} util={util:.2f}"
            ws.cell(row=rr, column=left + 1, value=label).alignment = Alignment(horizontal="right", vertical="center", wrap_text=True)
            ws.cell(row=rr, column=left + 1).font = Font(size=9, bold=True)

    # Draw TRANS beams (yellow segments), highlight worst in red
    for tb in transfer_beams:
        is_worst = (tb.tb_id == worst_id)
        f = fill_worst if is_worst else fill_trans

        if tb.span_dir == "X":
            rr = y_to_r(tb.fixed_coord)
            cL = x_to_c(tb.left_pos)
            cR = x_to_c(tb.right_pos)
            for cc in range(min(cL, cR), max(cL, cR) + 1):
                ws.cell(row=rr, column=cc).fill = f
                ws.cell(row=rr, column=cc).border = border_med_bottom
            # label at midpoint
            cm = (cL + cR) // 2
            ws.cell(row=rr - 1, column=cm, value=tb.tb_id).font = Font(size=8, bold=True)
            ws.cell(row=rr - 1, column=cm).alignment = Alignment(horizontal="center")
        else:
            cc = x_to_c(tb.fixed_coord)
            rB = y_to_r(tb.left_pos)
            rT = y_to_r(tb.right_pos)
            for rr in range(min(rB, rT), max(rB, rT) + 1):
                ws.cell(row=rr, column=cc).fill = f
                ws.cell(row=rr, column=cc).border = border_med_right
            rm = (rB + rT) // 2
            ws.cell(row=rm, column=cc + 1, value=tb.tb_id).font = Font(size=8, bold=True)
            ws.cell(row=rm, column=cc + 1).alignment = Alignment(horizontal="left")

    # Point load markers (actual location)
    for pl in cfg.loads:
        rr = y_to_r(pl.y)
        cc = x_to_c(pl.x)
        ws.cell(row=rr, column=cc, value=f"●{pl.load_id}").font = Font(color="FF0000", bold=True)

    # Allocation markers on receiving MAIN beams (▲ at the same span coordinate)
    # Map load_id -> (x, y)
    load_xy = {pl.load_id: (pl.x, pl.y) for pl in cfg.loads}
    pos_by_beam = {b.beam_id: b.pos for b in main_beams}
    for load_id, support_id, _p in alloc_rows:
        if support_id == "PERIM" or support_id not in pos_by_beam:
            continue
        x, y = load_xy.get(load_id, (None, None))
        if x is None:
            continue
        if best.direction == "Y":
            rr = y_to_r(y)
            cc = x_to_c(pos_by_beam[support_id])
        else:
            rr = y_to_r(pos_by_beam[support_id])
            cc = x_to_c(x)
        ws.cell(row=rr, column=cc, value=f"▲{load_id}").font = Font(color="1F4E79", bold=True)

    # Legend
    legend_r = r1 + 2
    ws.cell(row=legend_r, column=left, value="Legend:").font = Font(bold=True)
    ws.cell(row=legend_r + 1, column=left, value="MAIN beam: light-blue fill + medium line")
    ws.cell(row=legend_r + 2, column=left, value="TRANS beam (added for off-beam point load): yellow fill + medium line")
    ws.cell(row=legend_r + 3, column=left, value="Worst utilization beam: red fill")
    ws.cell(row=legend_r + 4, column=left, value="Point load: red ●ID, Allocation marker on receiving beam: ▲ID")

    return legend_r + 6


def write_beam_checks_table(
    ws,
    row0: int,
    col0: int,
    main_beams: List[Beam],
    ...
    """Write per-beam M/V/d/util table. Returns next row."""
    ws.cell(row=row0, column=col0, value="Beam checks (per member)").font = Font(bold=True)

    headers = [
        "BeamID", "Type", "Dir",
        "Pos/Fixed [m]", "Left [m]", "Right [m]", "Span [m]",
        "w_udl [kN/m]", "Npoint",
        "Mmax [kN*m]", "Vmax [kN]", "dmax [mm]",
        "util_M", "util_V", "util_d", "util_max", "OK/NG"
    ]
    for j, h in enumerate(headers):
        write_header(ws, row0 + 1, col0 + j, h)

    # build rows
    rows = []
    for b in main_beams:
        bc = checks.get(b.beam_id)
        if bc is None:
            continue
        rows.append((
            bc.beam_id, bc.beam_type, bc.direction,
            b.pos, "", "", bc.span,
            bc.w_udl, bc.n_point,
            bc.Mmax, bc.Vmax, bc.dmax * 1000.0,
            bc.util_M, bc.util_V, bc.util_d, bc.util_max,
            "OK" if bc.util_max <= 1.0 else "NG"
        ))
    for tb in transfer_beams:
        bc = checks.get(tb.tb_id)
        if bc is None:
            continue
        rows.append((
            bc.beam_id, bc.beam_type, bc.direction,
            tb.fixed_coord, tb.left_pos, tb.right_pos, bc.span,
            bc.w_udl, bc.n_point,
            bc.Mmax, bc.Vmax, bc.dmax * 1000.0,
            bc.util_M, bc.util_V, bc.util_d, bc.util_max,
            "OK" if bc.util_max <= 1.0 else "NG"
    ...
    # stable sorting: MAIN first then TRANS
    def sort_key(r):
        t = r[1]
        return (0 if t == "MAIN" else 1, str(r[0]))
    rows.sort(key=sort_key)

    worst_fill = PatternFill("solid", fgColor="F8CBAD")
    for i, r in enumerate(rows, start=0):
        rr = row0 + 2 + i
        for j, v in enumerate(r):
            ws.cell(row=rr, column=col0 + j, value=v)
        if r[0] == worst_id:
            for j in range(len(headers)):
                ws.cell(row=rr, column=col0 + j).fill = worst_fill

    return row0 + 3 + len(rows)


def write_layout_sheet(wb: Workbook, cfg: Config, mat: Material, setts: SolverSettings, best_A: Optional[CandidateResult], best_B: Optional[CandidateResult]) -> None:
    ...
    for label, best in [("A", best_A), ("B", best_B)]:
        if best is None:
            ws.cell(row=r, column=1, value=f"Mode-{label}: NO FEASIBLE SOLUTION").font = Font(bold=True)
            r += 4
            continue

        # attach info used in titles
        best.mode_label = label
        best.mode_desc = "total weight minimum" if label == "A" else "minimize maximum section rank"

        section = Section(rank=best.section_rank, name=best.section_name, w_g=best.w_g)

        main_beams, transfer_beams, alloc_rows, checks, worst_id = compute_system_checks(
            cfg, mat, setts, best.direction, best.pitch, section
        )

        r = draw_plan(ws, r, 1, cfg, best, main_beams, transfer_beams, alloc_rows, checks, worst_id)
        r = write_beam_checks_table(ws, r, 1, main_beams, transfer_beams, checks, worst_id)

        # Allocation table
        r += 1
        ws.cell(row=r, column=1, value="Load allocation to supports (includes TRANS reactions)").font = Font(bold=True)
        write_header(ws, r + 1, 1, "LoadID")
        write_header(ws, r + 1, 2, "SupportID")
        write_header(ws, r + 1, 3, "AllocatedP [kN]")
        rr = r + 2
        for load_id, sup_id, p in alloc_rows:
            ws.cell(row=rr, column=1, value=load_id)
            ws.cell(row=rr, column=2, value=sup_id)
            ws.cell(row=rr, column=3, value=p)
            rr += 1
        r = rr + 3
...
def write_result_xlsx(
    ...
    def write_member_list(title: str, best: Optional[CandidateResult], row0: int) -> int:
        ws.cell(row=row0, column=1, value=title).font = Font(bold=True)
        write_header(ws, row0 + 1, 1, "MemberType")
        write_header(ws, row0 + 1, 2, "Section")
        write_header(ws, row0 + 1, 3, "Count")
        write_header(ws, row0 + 1, 4, "Span/TotalLen [m]")
        write_header(ws, row0 + 1, 5, "w_g [kN/m]")
        write_header(ws, row0 + 1, 6, "Weight [kN]")

        if best is None:
            ws.cell(row=row0 + 2, column=1, value="NO FEASIBLE SOLUTION")
            return row0 + 4

        span_main = cfg.Lx if best.direction == "X" else cfg.Ly
        w_g = best.w_g

        w_main = w_g * span_main * best.n_beams
        w_trans = w_g * best.transfer_total_length

        rr = row0 + 2
        ws.cell(row=rr, column=1, value="MAIN")
        ws.cell(row=rr, column=2, value=best.section_name)
        ws.cell(row=rr, column=3, value=best.n_beams)
        ws.cell(row=rr, column=4, value=round(span_main, 6))
        ws.cell(row=rr, column=5, value=round(w_g, 6))
        ws.cell(row=rr, column=6, value=round(w_main, 6))
        rr += 1

        ws.cell(row=rr, column=1, value="TRANS")
        ws.cell(row=rr, column=2, value=best.section_name)
        ws.cell(row=rr, column=3, value=best.n_transfer)
        ws.cell(row=rr, column=4, value=round(best.transfer_total_length, 6))
        ws.cell(row=rr, column=5, value=round(w_g, 6))
        ws.cell(row=rr, column=6, value=round(w_trans, 6))
        rr += 1

        ws.cell(row=rr, column=1, value="TOTAL")
        ws.cell(row=rr, column=6, value=round(best.total_weight, 6))
        return rr + 2
    ...
    write_layout_sheet(wb, cfg, mat, setts, best_A, best_B)
